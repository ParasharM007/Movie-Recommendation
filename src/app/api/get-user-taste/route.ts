  ...
  try {
    ...
    if (!user || !user._id || !session)
     ...
     if(!getUserTasteResponse ) return apiResponse(false,"Error in getting user taste",400)
      ...
      getUserTasteResponse[safeField] && console.log("Field data:- ",getUserTasteResponse[safeField])
      //now we will check if field is likedGenres or not :- 
      if(safeField==='likedGenres') {
        return apiResponse(true, "Fetched Liked Genres",200,getUserTasteResponse[safeField])
      }
      ...
      // we will create func. that search with tmdb id:-
         ...
         async function searchMovieByIdWithTMDB(id:string, ) {
          ...
          const res = await axios.get(`${TMDB_BASE_URL}/movie/${movieId}?api_key=${TMDB_API_KEY}&language=en-US`, {
       
      });

      if (!res.data) return null;
       
      const temp = res.data.results?.[0] || null 
      if(!temp) return null
